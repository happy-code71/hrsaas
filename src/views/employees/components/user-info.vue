...
export default {
    ...
    methods: {
        async getUserDetailById() {
            ...
            // 当获取下来的图片为 null 时，说明没有图片，此时就不显示了
            if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
                this.$refs['userImg'].file_list = [{ url: this.userInfo.staffPhoto.trim(), upload: true }]
            }
        },
        async getPersonalDetail() {
            this.formData = await getPersonalDetail(this.userId)
            if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
                this.$refs['userImg2'].file_list = [{ url: this.formData.staffPhoto.trim(), upload: true }]
            }
        },
        async saveUser() {
            if (!this.$refs['userImg'].file_list.every(item => item.upload)) {
                this.$message.error('您当前还有图片没有上传完成！')
                return
            }
            // 当图片被删除之后，就没有图片了，这时用' ' 代替, 因为是空串的话不可以更改的
            await saveUserDetailById({ ...this.userInfo, staffPhoto: this.$refs['userImg'].file_list.length ? this.$refs['userImg'].file_list[0].url : ' ' })
            this.$message.success('更新用户详情成功')
        },
        async savePersonal() {
            if (!this.$refs['userImg2'].file_list.every(item => item.upload)) {
                this.$message.error('您当前还有图片没有上传完成！')
                return
            }
            await updatePersonal({ ...this.formData, staffPhoto: this.$refs['userImg2'].file_list.length ? this.$refs['userImg2'].file_list[0].url : ' ' })
            this.$message.success('更新用户详情成功')
        }
    }
}
