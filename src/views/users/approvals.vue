<template>
  <div class="usersApprovalsContainer">
    ...
    <div class="approvalsContent">
      <div class="topTitle">
        <div>
          ...
          <el-radio-group v-model="approvalsTypes" style="margin:5px 0">
            <el-radio v-for="item in approvalsType" :key="item.deploymentId" :label="item.key" :value="item.key"
        ...
        <div v-if="tagName != 'approvals'">
          ...
          <el-checkbox-group v-model="approvalsStates">
            <el-checkbox v-for="item in approvalsState" :key="item.id" :label="item.id" :value="item.id"
      ...
      <div>
        ...
        <el-table :data="tableData" style="width: 100%" :default-sort="{ order: 'descending', prop: 'procApplyTime' }">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-button v-show="(tagName == 'launch') && (scope.row.processState === '1')" size="mini" type="text"
              ...
              <el-button v-show="(tagName == 'copy' || tagName == 'approvals') && (scope.row.processState === '1')"
                size="mini" type="text" @click="clickPass('2', scope.row.processId)">通过</el-button>
              <!--  && item.row.currentApproverId == userId -->
              <el-button v-show="(tagName == 'copy' || tagName == 'approvals') && (scope.row.processState === '1')"
                size="mini" type="text" @click="clickPass('3', scope.row.processId)">驳回</el-button>
              <el-button size="mini" type="text" @click="clickDetail(scope.row.processId, scope.row.processName)">查看
              </el-button>
        ...
        <el-row type="flex" justify="center" style="height:60px" align="middle">
          <el-pagination :total="Number(total)" :page-sizes="[10, 20, 30, 50]" layout="prev, pager, next"
    ...
    <el-dialog :title="topLabel" :visible.sync="centerDialogVisible" width="50%" left>
      ...
      <Quit v-show="seeState == 'quit'" ref="quit" :selected-id="selectedId" :tab-lab="tagName"
        @closeDialog="closeDialog" />
      <!-- <Examine v-show="seeState == 'examine'" /> -->
      <Leave v-show="seeState == 'leave'" ref="leave" :selected-id="selectedId" :tab-lab="tagName"
        @closeDialog="closeDialog" />
      <Overtime v-show="seeState == 'overtime'" ref="overtime" :selected-id="selectedId" :tab-lab="tagName"
        @closeDialog="closeDialog" />
